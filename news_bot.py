...
def save_keywords_github(kw: dict):
    """GitHub API로 keywords.json 업데이트."""
    if not GITHUB_TOKEN or not GITHUB_REPO:
        print("GitHub 토큰 없음 - 로컬만 저장")
        save_keywords_local(kw)
        return
    url = f"https://api.github.com/repos/{GITHUB_REPO}/contents/keywords.json"
    headers = {
    ...
    try:
        ...
        content = base64.b64encode(
            json.dumps(kw, ensure_ascii=False, indent=2).encode()
        ).decode()
        body = {
            "message": f"키워드 업데이트 ({datetime.now().strftime('%m-%d %H:%M')})",
            "content": content,
        }
        if sha:
            body["sha"] = sha
        requests.put(url, json=body, headers=headers, timeout=10)
        print("GitHub keywords.json 업데이트 완료")
